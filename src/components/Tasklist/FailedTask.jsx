      ...
      const updatedEmployees = localData.employee.map((employee) => {
        if (employee.id === employeeId) {
          return {
            ...
            taskCounts: {
              ...employee.taskCounts,
              failed: Math.max((employee.taskCounts.failed || 0) - 1,0), // Update failed count
            },
          };
        }
        return employee;
  ...
  return (
    ...
    <div className="flex-shrink-0 w-[300px] min-h-64 p-5 bg-blue-500 rounded-xl relative">
      ...
      <div className="mt-6 flex gap-7">
        <button className="w-full bg-red-500 text-white rounded-lg font-medium py-2 text-sm">
          Failed
        </button>
      </div>

      {/* Delete Button (Positioned Separately from Date) */}
      <button
